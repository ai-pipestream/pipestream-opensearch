package ai.pipestream.schemamanager.grpc;
...
/**
 * Client-side workaround for Quarkus PR #54084 (BlockingServerInterceptor
 * replay-queue ordering bug).
 *
 * <p>When the server's blocking interceptor defers {@code next.startCall}
 * onto a backlogged worker / virtual thread, the deframer can deliver
 * {@code onHalfClose} before the application listener is allowed to see
 * {@code onMessage}. The underlying {@code UnaryServerCallListener} then
 * sees a half-close with {@code request == null} and closes the call with
 * {@code Status.INTERNAL: Half-closed without a request}. The handler
 * never runs, so a fresh attempt with the same request payload is safe.
 *
 * <p>This interceptor catches exactly that status code + description on
 * <b>unary</b> calls and retries the call once. The retry creates a new
 * underlying {@link ClientCall} and replays the original headers, request
 * message, and half-close. By the time the second attempt arrives at the
 * server the VT executor has caught up and the race window is closed.
 *
 * <p>Scope is deliberately narrow:
 * <ul>
 *   <li>Only unary RPCs (streaming RPCs would require replaying every
 *       message, which is more invasive — the bug doesn't trigger on
 *       long-lived streams anyway because there's no per-call buffer
 *       churn at the framework level).</li>
 *   <li>Only the exact {@code INTERNAL: Half-closed without a request}
 *       signature. Any other failure passes through unmodified so real
 *       application errors aren't masked.</li>
 *   <li>One retry maximum. If the second attempt also fails, the original
 *       failure propagates and the caller sees it.</li>
 * </ul>
 *
 * <p>Marked {@link GlobalInterceptor} so Quarkus wires it onto every
 * {@code @GrpcClient}-injected channel automatically — no per-stub
 * configuration needed.
 *
 * <p>REMOVAL: delete this class once the BOM is upgraded to a
 * quarkus-grpc release that contains the fix from PR #54084. The retry
 * will then be a no-op (the target status never fires), but keeping it
 * adds latency on real INTERNAL errors that happen to share the
 * description string, so prefer to remove it.
 */
@ApplicationScoped
@GlobalInterceptor
public class HalfClosedRetryClientInterceptor implements ClientInterceptor {
     ...
     * BlockingServerInterceptor race condition. Compared with
