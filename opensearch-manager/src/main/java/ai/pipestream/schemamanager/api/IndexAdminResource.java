 ...
 * (testing sidecar, admin dashboard). Blocking on virtual threads.
 */
@Path("/api/v1/indices")
...
public class IndexAdminResource {
    ...
    @GET
    @RunOnVirtualThread
    public Response listIndices(@QueryParam("prefix") String prefix) {
        ListIndicesRequest.Builder req = ListIndicesRequest.newBuilder();
        if (prefix != null && !prefix.isBlank()) {
            req.setPrefixFilter(prefix);
        }
        try {
            ListIndicesResponse resp = indexingService.listIndices(req.build());
    ...
    @RunOnVirtualThread
    public Response getIndexStats(@PathParam("indexName") String indexName) {
        try {
            GetIndexStatsResponse resp = indexingService.getIndexStats(
                    GetIndexStatsRequest.newBuilder().setIndexName(indexName).build());
    ...
    @RunOnVirtualThread
    public Response indexExists(@PathParam("indexName") String indexName) {
        IndexExistsResponse resp = indexingService.indexExists(
                IndexExistsRequest.newBuilder().setIndexName(indexName).build());
    ...
    @RunOnVirtualThread
    public Response getIndexMapping(@PathParam("indexName") String indexName) {
        try {
    ...
    @POST
    @RunOnVirtualThread
    public Response createIndex(Map<String, Object> payload) {
        String indexName = (String) payload.get("indexName");
        if (indexName == null || indexName.isBlank()) {
            return Response.status(400).entity(Map.of("error", "Missing 'indexName'")).build();
        }
    ...
    public Response deleteIndex(
        ...
        try {
            DeleteIndexResponse resp = indexingService.deleteIndex(
    ...
    public Response getDocument(
        ...
        GetOpenSearchDocumentResponse resp = indexingService.getOpenSearchDocument(
    ...
    public Response deleteDocument(
        ...
        DeleteDocumentResponse resp = indexingService.deleteDocument(
